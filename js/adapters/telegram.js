...
export class TelegramAdapter {
  ...
  async #loadManifest() {
    ...
    if (!pinned || typeof pinned.text !== "string") {
      this._manifest = { log: null, snapshot: null, blobIndex: null, manifestMsgId: null };
    ...
    this._manifest = {
      ...
      blobIndex: parsed.blobIndex || null,
  ...
  async #loadBlobIndex() {
    const m = await this.#loadManifest();
    if (!m.blobIndex || !m.blobIndex.fileId) return {};
    const text = await this.#downloadFile(m.blobIndex.fileId);
    if (!text) return {};
    try { return JSON.parse(text); } catch { return {}; }
  }

  async #saveBlobIndex(idx) {
    const filename = `${this.prefix.replace(/\//g, "_")}_blobs.json`;
    const up = await this.#uploadDocument(filename, JSON.stringify(idx));
    const m = await this.#loadManifest();
    await this.#saveManifest({ ...m, blobIndex: { fileId: up.fileId, messageId: up.messageId } });
  }

  async #uploadBlobDoc(filename, blob) {
    const fd = new FormData();
    fd.append("chat_id", this.chatId);
    fd.append("disable_notification", "true");
    ...
    const j = await r.json().catch(() => ({}));
    if (!r.ok || j.ok === false) {
      throw new Error(`Telegram sendDocument: ${j.description || r.status}`);
    }
    const doc = j.result && j.result.document;
    if (!doc || !doc.file_id) throw new Error("Telegram sendDocument: missing file_id");
    return { fileId: doc.file_id, messageId: j.result.message_id };
  ...
  async #downloadBlobDoc(fileId) {
    const meta = await this.#call("getFile", { file_id: fileId });
    if (!meta || !meta.file_path) throw new Error("Telegram getFile: missing file_path");
    const r = await fetch(`${API}/file/bot${this.token}/${meta.file_path}`);
    if (r.status === 404) return null;
    if (!r.ok) throw new Error(`Telegram blob download ${r.status}`);
    return await r.blob();
  }

  async putBlob(key, blob) {
    const filename = key.split("/").pop() || "blob";
    const up = await this.#uploadBlobDoc(filename, blob);
    const idx = await this.#loadBlobIndex();
    idx[key] = { fileId: up.fileId, messageId: up.messageId };
    await this.#saveBlobIndex(idx);
  }
  ...
  async getBlob(key) {
    ...
    return await this.#downloadBlobDoc(entry.fileId);
  }
