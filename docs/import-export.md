...
CODA Settings can read and write OPML 2.0 — the lingua franca of feed-reader exports. After this PR, you do not need to edit `worker/wrangler.toml` to change your subscription list. You upload an OPML file in the browser, the file is written to your storage adapter at `coda/subs/subscriptions.json`, and the next page reload fetches each feed via the same-origin `/fetch` proxy and renders the entries. Users on S3 + R2 still get the §4 Worker cron path; everyone else gets the browser engine in `js/feed-engine.js`.
...
6. Reload the page. `js/feed-engine.js` reads your subscriptions, fetches each feed through the same-origin `/fetch` proxy (Pages Function at `functions/fetch.js`, identical handler to the §4 Worker), parses with the shared parser, and renders the merged entries in the article list. The S3 + R2 path also keeps working: if a Worker-written `coda/feeds/snapshot.json` is available, it takes precedence and the browser engine is skipped.
...
The §4 Worker only reads the R2 path. The browser feed engine (`js/feed-engine.js`) reads from whichever adapter is active, so OPML imports produce visible entries on every adapter. Two requirements for the browser engine to work: (a) the same-origin `/fetch` proxy is deployed (the Cloudflare Pages Function in `functions/fetch.js` does this automatically), and (b) the `PROXY_ALLOW` environment variable on the Pages project is set to `*` or to an explicit URL-prefix list. With `PROXY_ALLOW` unset, every fetch returns 403 and the reader falls back to SAMPLE.
...
| Import succeeds but no entries appear after reload | `PROXY_ALLOW` is unset or too restrictive on the Pages project | Set `PROXY_ALLOW=*` (or a prefix list covering your feeds) in the Cloudflare Pages env vars; check the browser console for `feed-engine: <url> failed` lines |
| Import succeeds, Worker keeps using `wrangler.toml` feeds | Only relevant on S3 + R2 with a deployed Worker. Worker hasn't tick'd yet, or your R2 bucket is different from the Worker's | (a) wait up to 30 min, (b) confirm the bucket name in `worker/wrangler.toml` matches the bucket your Settings page is configured against |
